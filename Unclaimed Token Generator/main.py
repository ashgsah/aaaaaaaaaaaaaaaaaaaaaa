...
def generate_members():
    ...
    try:
        money = balance()
        ctypes.windll.kernel32.SetConsoleTitleW(f'Token Creator | Generated ~ {generated} | Captcha Geçenler ~ {cap_solved} | Hatalar ~ {errors} | Proxy Hatası ~ {proxy_error} | dc: clathrus')
        ...
        if response.status_code == 200:
            ...
            with output_lock:
                time_rn = get_time_rn()
                print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({lightblue}~{gray}) {pink}Bekleniyor {gray}| {green}{fingerprint}")
        else:
            with output_lock:
                time_rn = get_time_rn()
                print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({red}-{gray}) {pink}İptal Edildi {gray}| {red}Bad Gateaway")
            errors += 1
            generate_members()
        ...
        with output_lock:
            time_rn = get_time_rn()
            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({yellow}*{gray}) {pink}Captcha Çözüldü {gray}| {magenta}{captcha_code}")
        ...
        if "token" not in response.text:
            if "retry_after" in response.text:
                with output_lock:
                    time_rn = get_time_rn()
                    print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({red}-{gray}) {pink}Limit {gray}| {red}{response.json().get('retry_after')}s")
                errors += 1
                proxy_error += 1
                failed += 1
        ...
        if token == None:
            ...
            with output_lock:
                time_rn = get_time_rn()
                print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pink}Üretildi {gray}| ", end='')
                sys.stdout.flush()
